...
struct uevent_buff {
	char event_msg[UEVENT_BUFF_SIZE];
...
#ifdef CONFIG_UEVENTS_RECORD
static void parse_event(struct kobj_uevent_env *uevent_env,
...
static void uevents_collect(struct kobj_uevent_env *uevent_env)
{
	...
	if (uevent_env->buflen > 0) {
		...
		pr_debug("Event { %s', '%s', '%s', '%s'}\n", uevent.action,
			 uevent.path, uevent.subsystem, uevent.device_name);
		getnstimeofday(&uevents_buff[m].uevent_report_time);
		snprintf(uevents_buff[m++].event_msg, UEVENT_BUFF_SIZE,
			 "%s, %s, %s, %s", uevent.action, uevent.path,
			 uevent.subsystem, uevent.device_name);
...
static int uevent_net_init(struct net *net)
{
	...
	mutex_unlock(&uevent_sock_mutex);
#ifdef CONFIG_UEVENTS_RECORD
	entry = proc_create("uevents_records", 0, NULL,
			    &uevents_records_fileops);
	if (!entry)
		printk(KERN_ERR
